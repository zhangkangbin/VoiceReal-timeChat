...
先在 LM Studio 中启动 Local Server，并加载 `qwen2.5-coder-14b-instruct`。服务端默认读取 `AI_PROVIDER=lmstudio`，不会调用云端 API。语音识别默认使用已验证可用的 `tiny`；下载完整 `small` 权重后可改为 `small` 提高准确率。
